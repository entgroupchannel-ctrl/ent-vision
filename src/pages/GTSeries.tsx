...
import { ArrowLeft, ExternalLink, Cpu, Thermometer, Wind, Shield, Zap, Server, Factory, Building, Home, Monitor, Download, Play, Filter, X, Search, FileText, Share2 } from "lucide-react";
                  ...
                  <div className="flex items-start gap-4 p-4 rounded-xl bg-secondary/40 border border-border">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2 mt-3">
                        <LineQRButton className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#06C755] text-white text-xs font-semibold hover:bg-[#05b04c] transition-colors">
                        ...
                        <button
                          onClick={() => {
                            const url = window.location.origin + "/gt-series?tab=gt1000";
                            if (navigator.share) {
                              navigator.share({ title: "GT1000 Podcast — ENT GROUP", url });
                            } else {
                              navigator.clipboard.writeText(url);
                              alert("คัดลอกลิงก์แล้ว!");
                            }
                          }}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border text-xs font-semibold text-muted-foreground hover:text-foreground hover:bg-secondary/60 transition-colors"
                        >
                          <Share2 size={13} /> แชร์
                        </button>
                      </div>
                ...
                <div className="p-5 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mt-3">
                      ...
                      <button
                        onClick={() => {
                          const url = "https://youtu.be/7wE6fxMpJB4";
                          if (navigator.share) {
                            navigator.share({ title: "GT1000 Intro — ENT GROUP", url });
                          } else {
                            navigator.clipboard.writeText(url);
                            alert("คัดลอกลิงก์แล้ว!");
                          }
                        ...
                        <div key={video.id} className="space-y-2">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-1.5">
                              <LineQRButton className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-[#06C755] text-white text-[10px] font-semibold hover:bg-[#05b04c] transition-colors">
                                <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><path d="M19.365 9.863c.349 0 .63.285.63.631 0 .345-.281.63-.63.63H17.61v1.125h1.755c.349 0 .63.283.63.63 0 .344-.281.629-.63.629h-2.386c-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.627-.63h2.386c.349 0 .63.285.63.63 0 .349-.281.63-.63.63H17.61v1.125h1.755zm-3.855 3.016c0 .27-.174.51-.432.596-.064.021-.133.031-.199.031-.211 0-.391-.09-.51-.25l-2.443-3.317v2.94c0 .344-.279.629-.631.629-.346 0-.626-.285-.626-.629V8.108c0-.271.173-.508.43-.595.06-.023.136-.033.194-.033.195 0 .375.104.495.254l2.462 3.33V8.108c0-.345.282-.63.63-.63.345 0 .63.285.63.63v4.771zm-5.741 0c0 .344-.282.629-.631.629-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.627-.63.349 0 .631.285.631.63v4.771zm-2.466.629H4.917c-.345 0-.63-.285-.63-.629V8.108c0-.345.285-.63.63-.63.348 0 .63.285.63.63v4.141h1.756c.348 0 .629.283.629.63 0 .344-.281.629-.629.629M24 10.314C24 4.943 18.615.572 12 .572S0 4.943 0 10.314c0 4.811 4.27 8.842 10.035 9.608.391.082.923.258 1.058.59.12.301.079.766.038 1.08l-.164 1.02c-.045.301-.24 1.186 1.049.645 1.291-.539 6.916-4.078 9.436-6.975C23.176 14.393 24 12.458 24 10.314"/></svg>
                                LINE
                              </LineQRButton>
                              <button
                                onClick={() => {
                                  ...
                                  if (navigator.share) {
                                    navigator.share({ title: `${video.title} — ENT GROUP`, url });
                                  } else {
                                    navigator.clipboard.writeText(url);
                                    alert("คัดลอกลิงก์แล้ว!");
                                  }
                                }}
                                className="inline-flex items-center gap-1 px-2 py-1 rounded-md border border-border text-[10px] font-semibold text-muted-foreground hover:text-foreground hover:bg-secondary/60 transition-colors"
                              >
                                <Share2 size={11} />
                              </button>
                            </div>
