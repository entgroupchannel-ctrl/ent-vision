// Gallery & dimension images
import ibox007Angle2 from "@/assets/products/ibox-007-angle2.png";
import ibox007Angle3 from "@/assets/products/ibox-007-angle3.png";
import ibox007Dimension from "@/assets/products/ibox-007-dimension.png";
import ibox601GtProAngle2 from "@/assets/products/ibox-601-gt-pro-angle2.png";
import ibox601GtProAngle3 from "@/assets/products/ibox-601-gt-pro-angle3.png";
import ibox601GtProDimension from "@/assets/products/ibox-601-gt-pro-dimension.png";
import ibox602GtProAngle2 from "@/assets/products/ibox-602-gt-pro-angle2.png";
import ibox602GtProAngle3 from "@/assets/products/ibox-602-gt-pro-angle3.png";
import ibox602GtProDimension from "@/assets/products/ibox-602-gt-pro-dimension.png";
import aeolusTl9Angle2 from "@/assets/products/aeolus-tl9-6l2c4p-angle2.png";
import aeolusTl9Dimension from "@/assets/products/aeolus-tl9-6l2c4p-dimension.png";
import aeolusSv9Angle2 from "@/assets/products/aeolus-sv9-6l2c-1p-angle2.png";
import aeolusSv9Dimension from "@/assets/products/aeolus-sv9-6l2c-1p-dimension.png";
import aeolusSva35Angle2 from "@/assets/products/aeolus-sva35-2l10c-angle2.png";
import aeolusSva35Dimension from "@/assets/products/aeolus-sva35-2l10c-dimension.png";
import aeolusSv9_6l2cDimension from "@/assets/products/aeolus-sv9-6l2c-dimension.png";
import ibox205Front from "@/assets/products/ibox-205-front.png";
import ibox205Side from "@/assets/products/ibox-205-side.png";

import ibox603Front from "@/assets/products/ibox-603-front.png";
import ibox603Rear from "@/assets/products/ibox-603-rear.png";
import ibox604Front from "@/assets/products/ibox-604-front.png";
import ibox604Rear from "@/assets/products/ibox-604-rear.png";
import vbox105Front from "@/assets/products/vbox-105-front.png";
import vbox105Rear from "@/assets/products/vbox-105-rear.png";
import ibox007proFront from "@/assets/products/ibox-007pro-front.png";
import ibox007proRear from "@/assets/products/ibox-007pro-rear.png";
import ibox104v2Front from "@/assets/products/ibox-104v2-front.png";
import ibox104v2Side from "@/assets/products/ibox-104v2-side.png";
import ibox601Gt1pFront from "@/assets/products/ibox-601-gt-1p-front.png";
import ibox601Gt1pRear from "@/assets/products/ibox-601-gt-1p-rear.png";
import ibox501Img from "@/assets/products/ibox-501.png";
import ibox502Img from "@/assets/products/ibox-502.png";
import ibox601ClassicImg from "@/assets/products/ibox-601-classic.png";
import ibox602ClassicImg from "@/assets/products/ibox-602-classic.png";
import ibox603ClassicImg from "@/assets/products/ibox-603-classic.png";
import ibox701Img from "@/assets/products/ibox-701.png";
import ibox801Img from "@/assets/products/ibox-801.png";
// Main product images
import ibox007Main from "@/assets/products/ibox-007-main.png";
import ibox601GtProMain from "@/assets/products/ibox-601-gt-pro-main.png";
import ibox601GtProFront2 from "@/assets/products/ibox-601-gt-pro-front2.png";
import ibox601GtProRear2 from "@/assets/products/ibox-601-gt-pro-rear2.png";
import ibox602GtProMain from "@/assets/products/ibox-602-gt-pro-main.png";
import aeolusTl9Main from "@/assets/products/aeolus-tl9-6l2c4p-main.png";
import aeolusSv9Main from "@/assets/products/aeolus-sv9-6l2c-1p-main.png";
import aeolusSva35Main from "@/assets/products/aeolus-sva35-2l10c-main.png";
import aeolusSv9_6l2c from "@/assets/products/aeolus-sv9-6l2c.png";
import aeolusSv9Vision from "@/assets/products/aeolus-sv9-vision.png";
import ibox103Img from "@/assets/products/ibox-103.png";
import ibox104v2Img from "@/assets/products/ibox-104v2.png";
import ibox603Img from "@/assets/products/ibox-603-2l4c-1p.png";
import ibox604Img from "@/assets/products/ibox-604-main.png";
import ibox205Img from "@/assets/products/ibox-205-4l2c2p.png";
import vbox105Img from "@/assets/products/vbox-105-2l6c.png";
import ibox103_2l2cImg from "@/assets/products/ibox-103-2l2c.png";
import aeolusTl9_2l2cImg from "@/assets/products/aeolus-tl9-2l2c.png";

export type ProductCategory = "all" | "hot" | "embedded" | "edge" | "vision" | "arm" | "classic";

export interface IBoxProduct {
  id: string;
  name: string;
  nameTH: string;
  category: ProductCategory[];
  subcategory?: string;
  cpu: string;
  cpuGen?: string;
  ram: string;
  storage: string;
  io: string;
  temp: string;
  lanPorts?: number;
  comPorts?: number;
  usbPorts?: number;
  poe?: boolean;
  pcie?: boolean;
  image: string;
  gallery?: string[];
  dimensionImage?: string;
  features: string[];
  applications: string[];
  priceRange?: string;
  isNew?: boolean;
  isHot?: boolean;
}

export const iboxProducts: IBoxProduct[] = [
  {
    id: "ibox-007",
    name: "IBOX-007",
    nameTH: "Fanless Industrial PC 10 LAN PoE",
    category: ["hot", "embedded"],
    subcategory: "Embedded Box PC",
    cpu: "Intel Core i9/i7/i5/i3 (12th Gen Alder Lake-S)",
    cpuGen: "12th Gen",
    ram: "Up to 64GB DDR4 3200MHz",
    storage: "M.2 NVMe + 4× SATA RAID",
    io: "8× USB, 4× COM, 10× LAN (8× PoE)",
    temp: "-20°C ~ 55°C",
    lanPorts: 10, comPorts: 4, usbPorts: 8, poe: true, pcie: true,
    image: ibox007Main,
    gallery: [ibox007Angle2, ibox007Angle3],
    dimensionImage: ibox007Dimension,
    features: [
      "Intel 12th Gen Core i9/i7/i5/i3 Desktop CPU (LGA1700)",
      "Intel Q670 Chipset พร้อม vPRO Support",
      "10× GbE LAN (8× PoE สำหรับ Machine Vision)",
      "4× Swappable 2.5\" SSD RAID 0/1/5/10",
      "Dual HDMI 4K@60Hz Display Output",
      "Fanless Aluminum Enclosure",
      "Wide DC 9-36V Input",
      "16-bit GPIO + Mini-PCIe สำหรับ WiFi/4G",
    ],
    applications: ["AI Inference", "Edge Computing", "Machine Vision", "Industrial Server"],
    priceRange: "฿95,000 - ฿150,000",
    isHot: true,
  },
  {
    id: "ibox-007pro",
    name: "IBOX-007PRO",
    nameTH: "High-Performance Industrial PC — 12th/13th/14th Gen LGA1700",
    category: ["hot", "embedded", "edge"],
    subcategory: "Embedded Box PC",
    cpu: "12th/13th/14th Gen Intel® Core™ i9/i7/i5/i3 (LGA1700, Alder/Raptor Lake)",
    cpuGen: "12th-14th Gen (Alder Lake / Raptor Lake)",
    ram: "2× Dual-CH DDR4-3200, Up to 64GB",
    storage: "4× SATA 3.0 (Swappable SSD, RAID 0/1/5/10) + 2× M.2 2280 M-key (NVMe/SATA) + 1× M.2 3052 B-key",
    io: "4-6× USB3.0, 4× USB2.0, 1× Internal USB2.0, 2× COM (RS232/422/485), 2× HDMI, 1× 16-bit GPIO, 1× 8-bit GPIO",
    temp: "-20°C ~ 55°C (w/ 0.7m/s Airflow)",
    lanPorts: 10, comPorts: 2, usbPorts: 9, poe: true, pcie: true,
    image: ibox007proFront,
    gallery: [ibox007proRear, ibox007Main],
    features: [
      "Intel® Q670 Chipset + 12th/13th/14th Gen Desktop LGA1700 — สูงสุด 24 Cores / 32 Threads",
      "10× GbE LAN: สูงสุด 8× PoE (IEEE802.3af, PSE, End-Span) + 2× Non-PoE",
      "4× Swappable 2.5\" SSD พร้อม RAID 0/1/5/10 สำหรับงาน Server",
      "Intel® UHD Graphics 730/770 สูงสุด 32 Execution Units",
      "รองรับ vPRO (AMT) สำหรับ Remote Management",
      "ขยายได้ด้วย I/O Daughter Card (4× PoE LAN + GPIO + USB3.0) และ Expansion Daughter Card (PCIe x16/x4/x8)",
      "GPIO 16-bit + 8-bit, Remote Switch, SIM Slot, Mini-PCIe (WiFi/4G)",
      "Fanless Modular Design, จ่ายไฟ 9-36V DC, ขนาด 294×194.5×293.8 mm, น้ำหนัก 7 KG",
    ],
    applications: ["AI Inference", "Edge Computing", "Machine Vision", "Industrial Server", "Surveillance NVR"],
    priceRange: "สอบถาม",
    isHot: true, isNew: true,
  },
  {
    id: "ibox-601-gt-pro",
    name: "IBOX-601-GT-PRO",
    nameTH: "Edge AI Industrial PC + GPU — 12th/13th/14th Gen LGA1700",
    category: ["hot", "embedded", "edge"],
    subcategory: "AI Edge Computing",
    cpu: "12th/13th/14th Gen Intel® Core™ i3/i5/i7/i9 (LGA1700)",
    cpuGen: "12th-14th Gen (Alder/Raptor Lake)",
    ram: "2× Dual-CH DDR4-3200, Up to 64GB",
    storage: "1× 2.5\" SATA + 4× SATA 3.0 (RAID 0/1/5/10) + 1× mSATA + 2× M.2 2280 M-key (NVMe/SATA)",
    io: "4× USB3.2 Gen2 (10Gb/s) + 4× USB3.0 + 1× Internal USB2.0, 2× COM (RS232/RS485), 1× DP + 1× HDMI, GPIO 8-bit",
    temp: "-10°C ~ 55°C (w/ 0.7m/s Airflow)",
    lanPorts: 9, comPorts: 2, usbPorts: 9, poe: true, pcie: true,
    image: ibox601GtProFront2,
    gallery: [ibox601GtProRear2, ibox601GtProMain, ibox601GtProAngle2, ibox601GtProAngle3],
    dimensionImage: ibox601GtProDimension,
    features: [
      "Intel® H770 Chipset + 12th/13th/14th Gen Desktop LGA1700 — TDP สูงสุด 65W",
      "PCIe 4.0 x16 สำหรับ GPU — รองรับ 240W (Optional 500W/1000W Power Module)",
      "9× GbE LAN: 3× RS232 + 6× PoE (Intel i210/i219/i226, PSE End-Span)",
      "4× USB3.2 Gen2x1 (10Gb/s) + 4× USB3.0 (5Gb/s)",
      "5× SATA 3.0 (RAID 0/1/5/10) + 2× M.2 NVMe + 1× mSATA",
      "Triple Display: 1× DP (4096×2160@60Hz) + 1× HDMI (4096×2304@24Hz) + Optional HDMI via Daughter Card",
      "2× SIM Slot, M.2 B-key (4G/5G), M.2 E-key (WiFi/BT), TPM Header",
      "Active+Passive Cooling, จ่ายไฟ 9-36 VDC, ขนาด 290.3×152.6×324 mm, น้ำหนัก 4.55 KG (ไม่รวม GPU)",
    ],
    applications: ["AI Inference", "Deep Learning", "Edge AI", "Machine Vision", "Industrial Server"],
    priceRange: "สอบถาม",
    isHot: true, isNew: true,
  },
  {
    id: "ibox-601-gt-1p",
    name: "IBOX-601-GT-1P",
    nameTH: "Industrial PC Fanless — 12th/13th/14th Gen LGA1700 (H770)",
    category: ["hot", "embedded", "edge"],
    subcategory: "Edge Computing",
    cpu: "12th/13th/14th Gen Intel® Core™ i3/i5/i7/i9 (LGA1700)",
    cpuGen: "12th-14th Gen (Alder/Raptor Lake)",
    ram: "2× Dual-CH DDR4-3200 MHz SO-DIMM, Up to 64GB",
    storage: "1× 2.5\" SATA Bay + 4× SATA 3.0 (RAID 0/1/5/10) + 1× mSATA + 2× M.2 2280 (NVMe/SATA)",
    io: "4× USB3.2 Gen2 (10Gb/s) + 4× USB3.0 + 1× Internal USB2.0, 1× RS232/485 (DB9) + 1× RS232/485 (Jumper), 1× DP + 1× HDMI, GPIO 8-bit",
    temp: "-10°C ~ 55°C (w/ 0.7m/s Airflow)",
    lanPorts: 8, comPorts: 2, usbPorts: 9, poe: true, pcie: true,
    image: ibox601Gt1pFront,
    gallery: [ibox601Gt1pRear],
    features: [
      "Intel® H770 Chipset + 12th/13th/14th Gen Desktop LGA1700 — i3 TDP 65W / i5-i9 TDP 35W",
      "1× PCIe 4.0 x16 Slot สำหรับ Expansion Card (ไม่มี GPU Module ในตัว)",
      "8× GbE LAN: 2× Intel i210AT + 1× Intel i219LM + 1× Intel i226 + 6× PoE (PSE End-Span, shared Mini-PCIe)",
      "4× USB3.2 Gen2x1 (10Gb/s) + 4× USB3.0 (5Gb/s) + 3× USB2.0",
      "5× SATA 3.0 (RAID 0/1/5/10) + 2× M.2 NVMe + 1× mSATA",
      "Dual Display: 1× DP (4096×2160@60Hz) + 1× HDMI (4096×2304@24Hz) + Optional HDMI via TP-IPC Daughter Card",
      "2× External SIM Slot + M.2 B-key (4G/5G LTE) + M.2 E-key (WiFi/BT), TPM Header",
      "Audio: 1× MIC + 1× SPK (7.1+2 CH, Realtek ALC897)",
      "จ่ายไฟ 9-36 VDC, ขนาด 290.35×109.28×190 mm, น้ำหนัก 4.25 KG",
    ],
    applications: ["Edge Computing", "Industrial Server", "Surveillance NVR", "IoT Gateway", "Digital Signage"],
    priceRange: "สอบถาม",
    isNew: true,
  },
  {
    id: "ibox-602-gt-pro",
    name: "IBOX-602-GT-PRO",
    nameTH: "GPU Industrial PC รองรับ RTX 3090/4090",
    category: ["hot", "embedded", "edge"],
    subcategory: "AI Edge Computing",
    cpu: "Intel Core i9/i7/i5/i3 (12th-14th Gen)",
    cpuGen: "12th-14th Gen",
    ram: "Up to 128GB DDR4",
    storage: "2× M.2 NVMe + 4× SATA RAID",
    io: "8× USB, 6× COM, 3× LAN",
    temp: "-10°C ~ 60°C",
    lanPorts: 3, comPorts: 6, usbPorts: 8, poe: false, pcie: true,
    image: ibox602GtProMain,
    gallery: [ibox602GtProAngle2, ibox602GtProAngle3],
    dimensionImage: ibox602GtProDimension,
    features: [
      "Intel 12th/13th/14th Gen Core Desktop CPU (LGA1700)",
      "Intel H770 Chipset",
      "PCIe 4.0 x16 รองรับ NVIDIA RTX 3090/4090",
      "3× GbE LAN (Intel i210/i219)",
      "4× Isolated COM + 2× RS232",
      "Dual M.2 NVMe + 4× SATA RAID Support",
      "DP 8K + HDMI 4K Display Output",
      "Wide DC 19-36V Input",
      "8-bit GPIO + Dual SIM สำหรับ 4G/5G",
    ],
    applications: ["AI Training", "Deep Learning", "3D Rendering", "Edge AI"],
    priceRange: "฿150,000 - ฿250,000",
    isHot: true, isNew: true,
  },
  {
    id: "aeolus-tl9-6l2c4p",
    name: "AEOLUS-TL9-6L2C4P",
    nameTH: "Machine Vision PC PoE + Light Controller",
    category: ["hot", "vision"],
    subcategory: "Machine Vision",
    cpu: "Intel Core i7/i5/i3 (8th/9th Gen)",
    cpuGen: "8th-9th Gen",
    ram: "Up to 32GB DDR4",
    storage: "2× M.2 2280 NVMe/SATA",
    io: "6× USB, 2× Isolated COM, 6× LAN",
    temp: "-10°C ~ 60°C",
    lanPorts: 6, comPorts: 2, usbPorts: 6, poe: true,
    image: aeolusTl9Main,
    gallery: [aeolusTl9Angle2],
    dimensionImage: aeolusTl9Dimension,
    features: [
      "Intel 8th/9th Gen Core i7/i5/i3 (LGA1151)",
      "Intel Z390 Chipset",
      "6× GbE LAN (4× PoE IEEE 802.3af 15W/port)",
      "4-Channel Light Source Controller (12V/24V)",
      "4-Channel External Trigger Signal",
      "2× Isolated COM (RS232/485) + ESD Protection",
      "16-bit GPIO (8-in/8-out, 5-24V)",
      "HDMI 4K + DP 4K Display Output",
    ],
    applications: ["Machine Vision", "AOI Inspection", "Robotics Vision", "Quality Control"],
    priceRange: "฿85,000 - ฿120,000",
    isHot: true, isNew: true,
  },
  {
    id: "aeolus-sv9-6l2c",
    name: "AEOLUS-SV9-6L2C",
    nameTH: "Machine Vision Industrial PC",
    category: ["vision"],
    subcategory: "Embedded Box PC",
    cpu: "Intel Core i7-1365URE (13th Gen)",
    cpuGen: "13th Gen",
    ram: "Up to 64GB DDR5",
    storage: "M.2 NVMe SSD",
    io: "6× USB 3.2, 2× COM, 6× LAN",
    temp: "-20°C ~ 60°C",
    lanPorts: 6, comPorts: 2, usbPorts: 6, poe: true, pcie: true,
    image: aeolusSv9_6l2c,
    gallery: [aeolusSv9Vision],
    dimensionImage: aeolusSv9_6l2cDimension,
    features: [
      "Intel 13th Gen Raptor Lake",
      "6× PoE+ GbE LAN",
      "Fanless Design",
      "Wide Temperature Range",
      "TPM 2.0 Support",
    ],
    applications: ["Machine Vision", "AI Edge Computing", "AOI Inspection"],
    priceRange: "฿85,000 - ฿120,000",
    isNew: true,
  },
  {
    id: "apollo-sv12-8p",
    name: "APOLLO-SV12-8P",
    nameTH: "Vision Controller 8 Port PoE",
    category: ["vision"],
    subcategory: "Vision Controller",
    cpu: "Intel Core i7-1185G7E (11th Gen)",
    cpuGen: "11th Gen",
    ram: "Up to 64GB DDR4",
    storage: "2× M.2 NVMe",
    io: "4× USB 3.2, 2× COM, 8× PoE LAN",
    temp: "-20°C ~ 60°C",
    lanPorts: 8, comPorts: 2, usbPorts: 4, poe: true, pcie: true,
    image: aeolusSv9Vision,
    gallery: [aeolusSv9_6l2c, aeolusTl9_2l2cImg],
    features: [
      "8× PoE+ GbE LAN สำหรับกล้อง Vision",
      "Intel 11th Gen Tiger Lake",
      "Fanless Industrial Grade",
      "TPM 2.0 + Secure Boot",
    ],
    applications: ["Machine Vision", "AOI Inspection", "Multi-Camera System"],
    priceRange: "฿95,000 - ฿130,000",
  },
  {
    id: "aeolus-sv9-6l2c-1p",
    name: "AEOLUS-SV9-6L2C-1P",
    nameTH: "Edge Controller 6 LAN + PCIe",
    category: ["hot", "edge"],
    subcategory: "Edge Controller",
    cpu: "Intel Core i7/i5/i3 (6th-9th Gen)",
    cpuGen: "6th-9th Gen",
    ram: "Up to 64GB DDR4",
    storage: "M.2 2280 NVMe/SATA",
    io: "5× USB, 2× COM, 6× LAN",
    temp: "-10°C ~ 60°C",
    lanPorts: 6, comPorts: 2, usbPorts: 5, poe: false, pcie: true,
    image: aeolusSv9Main,
    gallery: [aeolusSv9Angle2],
    dimensionImage: aeolusSv9Dimension,
    features: [
      "Intel 6th-9th Gen Core i7/i5/i3 (LGA1151) up to 65W",
      "Intel H110 Chipset",
      "6× GbE LAN (5× Intel i210 + 1× i219) WoL/PXE",
      "1× PCIe x8 Expansion Slot",
      "2× RS232/RS485 Switchable via BIOS",
      "Mini-PCIe + Nano SIM สำหรับ 4G LTE",
      "Wide DC 12V/19V/24V Auto-detect",
      "Wall/DIN-rail/Vertical Mount Support",
    ],
    applications: ["Industrial Gateway", "Edge Computing", "Network Appliance", "Transportation"],
    priceRange: "฿65,000 - ฿95,000",
    isHot: true, isNew: true,
  },
  {
    id: "kronos-sv6-4l8c",
    name: "KRONOS-SV6-4L8C",
    nameTH: "Multi-LAN Industrial PC 6 LAN",
    category: ["edge"],
    subcategory: "Network Appliance",
    cpu: "Intel Core i7/i5/i3 (LGA1151)",
    cpuGen: "6th-9th Gen",
    ram: "Up to 64GB DDR4",
    storage: "M.2 NVMe + SATA 3.0",
    io: "4× USB 3.0, 2× COM, 6× LAN",
    temp: "-20°C ~ 60°C",
    lanPorts: 6, comPorts: 2, usbPorts: 4, poe: false, pcie: true,
    image: aeolusSv9_6l2c,
    gallery: [aeolusSv9Vision],
    dimensionImage: aeolusSv9_6l2cDimension,
    features: [
      "Intel 6th-9th Gen Core",
      "6× Intel Gigabit LAN",
      "Active Cooling Design",
      "Wide DC 12V/19V/24V Input",
      "Mini-PCIe + Nano SIM",
    ],
    applications: ["Network Appliance", "Edge Gateway", "Industrial IoT"],
    priceRange: "฿65,000 - ฿85,000",
  },
  {
    id: "aeolus-sva35-2l10c",
    name: "AEOLUS-SVA35-2L10C",
    nameTH: "ARM Edge Controller 10× RS485 + AI NPU",
    category: ["hot", "arm"],
    subcategory: "ARM Controller",
    cpu: "Rockchip RK3568 Quad-Core ARM Cortex-A55",
    cpuGen: "ARM",
    ram: "4GB/8GB LPDDR4X",
    storage: "64GB eMMC + M.2 NVMe",
    io: "3× USB, 10× RS485, 2× CAN, 2× LAN",
    temp: "-10°C ~ 60°C",
    lanPorts: 2, comPorts: 10, usbPorts: 3,
    image: aeolusSva35Main,
    gallery: [aeolusSva35Angle2],
    dimensionImage: aeolusSva35Dimension,
    features: [
      "Rockchip RK3568 Quad-core ARM Cortex-A55 @ 2.0GHz",
      "1T NPU สำหรับ AI Acceleration",
      "ARM G52 2EE GPU รองรับ OpenGL/Vulkan",
      "10× RS485 (2 ช่องรองรับ RS232/RS485)",
      "2× Isolated CAN Bus",
      "8-bit Isolated GPIO (4 DI + 4 DO)",
      "2× GbE LAN + M.2 B-Key สำหรับ 4G/5G",
      "Wide DC 9-28V Input",
      "Fanless Enclosed Design",
      "รองรับ Linux และ Android 11",
    ],
    applications: ["IoT Gateway", "Industrial Automation", "Building Automation", "Energy Monitoring"],
    priceRange: "฿35,000 - ฿55,000",
    isHot: true, isNew: true,
  },
  {
    id: "ibox-103",
    name: "IBOX-103",
    nameTH: "Compact Fanless Mini PC",
    category: ["embedded"],
    subcategory: "Compact Box PC",
    cpu: "Intel Celeron J4125 / N5105",
    cpuGen: "Gemini Lake / Jasper Lake",
    ram: "Up to 16GB DDR4",
    storage: "M.2 SATA + 2.5\" SATA",
    io: "4× USB, 2× COM, 2-4× LAN",
    temp: "-10°C ~ 60°C",
    lanPorts: 4, comPorts: 2, usbPorts: 4,
    image: ibox103Img,
    gallery: [ibox103_2l2cImg],
    features: [
      "Ultra-compact Fanless Design",
      "2-4× GbE LAN",
      "Low Power TDP 10W",
      "DIN-rail Mount Support",
    ],
    applications: ["Edge Gateway", "Digital Signage", "Thin Client"],
    priceRange: "฿15,000 - ฿25,000",
  },
  {
    id: "ibox-104v2",
    name: "IBOX-104V2-2L6C-XP",
    nameTH: "Ultra-thin Fanless Embedded PC — Core i5-3317U",
    category: ["embedded"],
    subcategory: "Compact Box PC",
    cpu: "Intel® Core™ i5-3317U Dual-core 1.70GHz (Turbo 2.60GHz), TDP 17W",
    cpuGen: "3rd Gen (Ivy Bridge)",
    ram: "1× SO-DIMM DDR3L-1333/1600, Up to 8GB",
    storage: "1× mSATA 3.0 (6Gb/s) + 1× SATA 2.0 (3Gb/s)",
    io: "6× COM (2× RS232/485 DB9 + 2× RS232/485 Header + 2× RS232 Header), 4× USB3.0, 5× USB2.0 (Wafer), 1× HDMI",
    temp: "-10°C ~ 50°C (w/ 0.7m/s Airflow)",
    lanPorts: 2, comPorts: 6, usbPorts: 9,
    image: ibox104v2Front,
    gallery: [ibox104v2Side, ibox104v2Img],
    features: [
      "Ultra-thin 44mm โครงอลูมิเนียมอัลลอย Fanless กันฝุ่น กันสั่นสะเทือน",
      "6× COM Ports (RS232/RS485) สำหรับอุปกรณ์อุตสาหกรรมหลากหลาย",
      "2× GbE LAN (Realtek RTL8111H) รองรับ Wake on LAN/PXE",
      "2× Mini-PCIe สำหรับ 4G-LTE, WiFi, BT, GNSS",
      "GPIO 8-bit (4-In/4-Out) + SIM Card Slot + TPM2.0",
      "รองรับ VESA และ Wall Mount ติดตั้งง่าย",
      "จ่ายไฟ 12V DC (Optional 9-30V DC), ขนาด 250×44×153 mm, น้ำหนัก 1.5 KG",
    ],
    applications: ["Industrial Automation", "POS System", "Kiosk", "Digital Signage", "IoT Gateway"],
    priceRange: "สอบถาม",
  },
  {
    id: "ibox-205",
    name: "IBOX-205-4L2C2P",
    nameTH: "Embedded Box PC 4 LAN (2 PoE) 2 COM — Tiger Lake 11th Gen",
    category: ["embedded"],
    subcategory: "Embedded Box PC",
    cpu: "11th Gen Intel® Core™ / Celeron® 6305E",
    cpuGen: "11th Gen (Tiger Lake)",
    ram: "2× SO-DIMM DDR4-3200, Up to 64GB",
    storage: "1× 2.5\" SATA 3.0 + 1× mSATA 3.0",
    io: "2× USB3.0, 2× USB2.0, 1× Internal USB2.0, 2× COM (RS232/485), 1× HDMI, 1× VGA, 1× GPIO 8-bit",
    temp: "-10°C ~ 55°C (w/ 0.7m/s Airflow)",
    lanPorts: 4, comPorts: 2, usbPorts: 5, pcie: true, poe: true,
    image: ibox205Front,
    gallery: [ibox205Side, ibox205Img],
    features: [
      "Intel® Iris® Xe Graphics — กราฟิกแรงกว่ารุ่นเดิมถึง 2.95 เท่า",
      "2× PoE LAN (Intel i211, IEEE802.3af) + 2× 2.5GbE LAN (Intel i225)",
      "2× Isolated COM พร้อม TVS Protection (RS232/485 สลับผ่าน BIOS)",
      "Fanless ระบายความร้อนแบบไร้พัดลม โครงอลูมิเนียมอัลลอยกันสัญญาณรบกวน",
      "รองรับ Mini-PCIe, M.2 2230 E-key, M.2 3052 B-key พร้อม SIM Slot",
      "GPIO 8-bit (4-In/4-Out), Remote Switch, Auto-On Switch",
      "ติดตั้งได้หลากหลาย: DIN-Rail, Wall-Mount, Embedded",
      "จ่ายไฟกว้าง 9-36 VDC, ขนาด 290.35×81.54×165.5 mm, น้ำหนัก 2.5 KG",
    ],
    applications: ["Industrial Automation", "Edge Computing", "IoT Gateway", "Machine Vision", "Digital Signage"],
    priceRange: "สอบถาม",
  },
  {
    id: "ibox-603-tpipc",
    name: "IBOX-603-2L4C-1P",
    nameTH: "Embedded Box PC 2 LAN 4 COM — 10th/11th Gen LGA1200",
    category: ["embedded"],
    subcategory: "Embedded Box PC",
    cpu: "10th/11th Gen Intel® Core™ i9/i7/i5/i3 (LGA1200)",
    cpuGen: "10th/11th Gen (Comet Lake-S)",
    ram: "2× Dual-CH DDR4-2933/2666, Up to 64GB",
    storage: "1× SATA 3.0 (6Gb/s) + 1× M.2 2280 (NVMe/SATA 3.0)",
    io: "4× USB3.2, 1× 9-Pin USB2.0, 4× COM (RS232/422/485), 2× HDMI 1.4, 1× DP, 1× GPIO 8-bit",
    temp: "-10°C ~ 60°C (w/ 0.7m/s Airflow)",
    lanPorts: 2, comPorts: 4, usbPorts: 5, pcie: true,
    image: ibox603Front,
    gallery: [ibox603Rear, ibox603Img],
    features: [
      "Intel® 10th/11th Gen Desktop LGA1200 — TDP สูงสุด 65W ประสิทธิภาพระดับ Desktop",
      "รองรับ Intel® Q470E Chipset พร้อม vPRO (Optional)",
      "4× COM (RS232/422/485) สำหรับอุปกรณ์อุตสาหกรรม",
      "Triple Display: 2× HDMI 1.4 + 1× DP (4096x2304@60Hz)",
      "Fanless โครงอลูมิเนียมอัลลอย ระบายความร้อนแบบไร้พัดลม",
      "รองรับ Half-Size PCIex16, M.2 E-key (CNVi), M.2 B-key (USB3.0/PCIe)",
      "จ่ายไฟ 12V-28V DC, ขนาด 290.3×87.58×186 mm, น้ำหนัก 3.9 KG",
    ],
    applications: ["Factory Automation", "Edge Computing", "POS System", "Kiosk", "Digital Signage"],
    priceRange: "สอบถาม",
  },
  {
    id: "ibox-604",
    name: "IBOX-604-5L2C2P",
    nameTH: "Embedded Box PC 5 LAN (2 PoE) 2 COM — 6th-9th Gen LGA1151",
    category: ["embedded"],
    subcategory: "Embedded Box PC",
    cpu: "6th/7th/8th/9th Gen Intel® Core™ i7/i5/i3 / Celeron® (LGA1151)",
    cpuGen: "6th-9th Gen (Skylake/Kaby Lake/Coffee Lake)",
    ram: "2× SO-DIMM DDR4-2400/2666, Up to 32GB",
    storage: "1× 2.5\" SATA 3.0 + 3× SATA 3.0 (RAID 0/1/5) + 1× mSATA + 1× M.2 2280 (NVMe/SATA)",
    io: "8× USB3.0, 1× Internal USB2.0, 2× COM (RS232/RS485), 1× HDMI, 1× VGA, 1× GPIO 8-bit",
    temp: "-10°C ~ 55°C (w/ 0.7m/s Airflow)",
    lanPorts: 5, comPorts: 2, usbPorts: 9, pcie: true, poe: true,
    image: ibox604Front,
    gallery: [ibox604Rear, ibox604Img],
    features: [
      "Intel® Z370 Chipset — Desktop LGA1151 รองรับ CPU สูงสุด 6 Cores, TDP 65W",
      "5× GbE LAN: 2× PoE (Intel i211, IEEE802.3af) + 3× GbE (Intel i211)",
      "2× Isolated COM (RS232/RS485 สลับผ่าน BIOS) + 2× RS232 shared GPIO",
      "รองรับ SSD Tray สูงสุด 4 ลูก พร้อม RAID 0/1/5",
      "Fanless โครงอลูมิเนียมอัลลอย พร้อม Surge Protection และ Ignition Power Control",
      "รองรับ M.2 B-key (SIM/USB3.0), M.2 E-key (WiFi/BT), Mini-PCIe (4G/5G LTE)",
      "จ่ายไฟ 9-36 VDC, ขนาด 290.35×87.58×186 mm, น้ำหนัก 3.9 KG",
    ],
    applications: ["Network Appliance", "Edge Gateway", "Firewall", "Factory Automation", "Surveillance"],
    priceRange: "สอบถาม",
  },
  {
    id: "vbox-105",
    name: "VBOX-105-2L6C",
    nameTH: "Ultra-compact Fanless Box PC — Celeron J6412 (Elkhart Lake)",
    category: ["embedded"],
    subcategory: "Compact Box PC",
    cpu: "Intel® Celeron® J6412 Quad-core 2.0GHz (Burst 2.6GHz), TDP 10W",
    cpuGen: "10th Gen (Elkhart Lake)",
    ram: "1× Non-ECC SO-DIMM DDR4-2666/2933/3200, Up to 32GB",
    storage: "1× SATA 3.0 + 1× Mini PCIe (mSATA) + 1× M.2 2280 M-key (NVMe/SATA) + 1× M.2 3052 B-key",
    io: "6× COM (3× RS232 + 2× RS232/TTL + 1× RS232/RS485/TTL), 3× USB3.0, 3× USB2.0 + 4× USB2.0 Header, 2× HDMI + 1× DP, GPIO 8-bit",
    temp: "-20°C ~ 65°C (w/ 0.7m/s Airflow)",
    lanPorts: 2, comPorts: 6, usbPorts: 10, pcie: true,
    image: vbox105Front,
    gallery: [vbox105Rear, vbox105Img],
    features: [
      "Intel® Celeron® J6412 Elkhart Lake — TDP เพียง 10W ประหยัดพลังงานสูง",
      "6× COM Ports (RS232/RS485/TTL) สำหรับอุปกรณ์อุตสาหกรรมหลากหลาย",
      "Triple Display: 2× HDMI (4096×2304@60Hz) + 1× DP (4096×2304@60Hz)",
      "รองรับ 4G/5G, WiFi, AI Accelerator ผ่าน M.2 และ Mini PCIe",
      "Software Ignition Control (ACC) พร้อม Surge Protection 80V",
      "Fanless โครงอลูมิเนียมอัลลอย ติดตั้ง DIN-Rail, Wall-Mount, VESA, Embedded",
      "จ่ายไฟกว้าง 6-42V DC, ขนาด 219.19×79.6×234 mm, น้ำหนัก 2 KG",
    ],
    applications: ["Digital Signage", "Vehicle Computing", "IoT Edge", "Thin Client", "AI Edge"],
    priceRange: "สอบถาม",
  },
  {
    id: "ibox-501",
    name: "iBox-501",
    nameTH: "Fanless Mini PC — Celeron Haswell 2980U",
    category: ["classic"],
    subcategory: "Classic iBox",
    cpu: "Intel Celeron Haswell 2980U Dual Core 1.6GHz",
    cpuGen: "4th Gen (Haswell)",
    ram: "4 GB DDR3L (ปรับแต่งได้)",
    storage: "SSD 128 GB (ปรับแต่งได้)",
    io: "USB, COM, LAN",
    temp: "0°C ~ 60°C",
    image: ibox501Img,
    features: [
      "Fanless Design ไร้พัดลม เงียบสนิท",
      "โครงอลูมิเนียมคุณภาพสูง ระบายความร้อนดีเยี่ยม",
      "Auto Power On เปิดเครื่องอัตโนมัติ",
      "Watchdog Timer ตรวจสอบความผิดปกติ",
      "รองรับ Windows 7/8/10 และ Linux",
    ],
    applications: ["Digital Signage", "Thin Client", "POS System"],
  },
  {
    id: "ibox-502",
    name: "iBox-502",
    nameTH: "Fanless Mini PC — Celeron Kaby Lake R 3867U",
    category: ["classic"],
    subcategory: "Classic iBox",
    cpu: "Intel Celeron Kaby Lake R 3867U Dual Core 1.8GHz",
    cpuGen: "7th Gen (Kaby Lake)",
    ram: "4 GB DDR4 (ปรับแต่งได้)",
    storage: "SSD 128 GB (ปรับแต่งได้)",
    io: "USB, COM, LAN",
    temp: "0°C ~ 60°C",
    image: ibox502Img,
    features: [
      "Fanless Design ไร้พัดลม เงียบสนิท",
      "โครงอลูมิเนียมคุณภาพสูง ระบายความร้อนดีเยี่ยม",
      "Auto Power On เปิดเครื่องอัตโนมัติ",
      "PXE Boot เปิดเครื่องผ่านระบบเครือข่าย",
      "รองรับ Windows 7/8/10 และ Linux",
    ],
    applications: ["Digital Signage", "Thin Client"],
  },
  {
    id: "ibox-601-classic",
    name: "iBox-601",
    nameTH: "Fanless Mini PC — Core 6th Gen Skylake",
    category: ["classic"],
    subcategory: "Classic iBox",
    cpu: "Intel Core 6th Gen Skylake",
    cpuGen: "6th Gen (Skylake)",
    ram: "4 GB DDR4 (ปรับแต่งได้)",
    storage: "SSD 128 GB (ปรับแต่งได้)",
    io: "USB, COM, LAN",
    temp: "0°C ~ 60°C",
    image: ibox601ClassicImg,
    features: [
      "Fanless Design ไร้พัดลม เงียบสนิท",
      "โครงอลูมิเนียมคุณภาพสูง ระบายความร้อนดีเยี่ยม",
      "PXE Boot เปิดเครื่องผ่านระบบเครือข่าย",
      "Timing Boot ตั้งเวลาเปิดเครื่องได้",
      "รองรับ Windows 7/8/10 และ Linux",
    ],
    applications: ["Factory Automation", "POS System"],
  },
  {
    id: "ibox-602-classic",
    name: "iBox-602",
    nameTH: "Fanless Mini PC — Core 6th Gen Skylake",
    category: ["classic"],
    subcategory: "Classic iBox",
    cpu: "Intel Core 6th Gen Skylake",
    cpuGen: "6th Gen (Skylake)",
    ram: "4 GB DDR4 (ปรับแต่งได้)",
    storage: "SSD 128 GB (ปรับแต่งได้)",
    io: "USB, COM, LAN",
    temp: "0°C ~ 60°C",
    image: ibox602ClassicImg,
    features: [
      "Fanless Design ไร้พัดลม เงียบสนิท",
      "โครงอลูมิเนียมคุณภาพสูง ระบายความร้อนดีเยี่ยม",
      "Watchdog Timer ตรวจสอบความผิดปกติ",
      "รองรับ Windows 7/8/10 และ Linux",
    ],
    applications: ["Industrial Automation", "Kiosk"],
  },
  {
    id: "ibox-603-classic",
    name: "iBox-603",
    nameTH: "Fanless Mini PC — Core 6th Gen Skylake (Pure Aluminum)",
    category: ["classic"],
    subcategory: "Classic iBox",
    cpu: "Intel Core 6th Gen Skylake",
    cpuGen: "6th Gen (Skylake)",
    ram: "4 GB DDR4 (ปรับแต่งได้)",
    storage: "SSD 128 GB (ปรับแต่งได้)",
    io: "USB, COM, LAN",
    temp: "0°C ~ 60°C",
    image: ibox603ClassicImg,
    features: [
      "Pure Aluminum Body อลูมิเนียมบริสุทธิ์",
      "Fanless Design ไร้พัดลม เงียบสนิท",
      "Timing Boot ตั้งเวลาเปิดเครื่องได้",
      "รองรับ Windows 7/8/10 และ Linux",
    ],
    applications: ["Digital Signage", "Automation"],
  },
  {
    id: "ibox-701",
    name: "iBox-701",
    nameTH: "Fanless Mini PC — Core i5-7200U",
    category: ["classic"],
    subcategory: "Classic iBox",
    cpu: "Intel Core i5-7200U",
    cpuGen: "7th Gen (Kaby Lake)",
    ram: "4 GB DDR4 (ปรับแต่งได้)",
    storage: "SSD 128 GB (ปรับแต่งได้)",
    io: "USB, COM, LAN",
    temp: "0°C ~ 60°C",
    image: ibox701Img,
    features: [
      "Core i5 Performance ประสิทธิภาพสูง",
      "Fanless Design ไร้พัดลม เงียบสนิท",
      "โครงอลูมิเนียมคุณภาพสูง",
      "รองรับ Windows 7/8/10 และ Linux",
    ],
    applications: ["Edge Computing", "Thin Client"],
  },
  {
    id: "ibox-801",
    name: "iBox-801",
    nameTH: "Fanless Mini PC — 8th Gen Core i5/i7",
    category: ["classic"],
    subcategory: "Classic iBox",
    cpu: "Intel Celeron / Core i5-8265U / Core i7-8565U",
    cpuGen: "8th Gen (Whiskey Lake)",
    ram: "8 GB DDR4 (ปรับแต่งได้)",
    storage: "SSD 256 GB (ปรับแต่งได้)",
    io: "USB, COM, LAN",
    temp: "0°C ~ 60°C",
    image: ibox801Img,
    features: [
      "8th Gen Core i5/i7 ประสิทธิภาพสูง",
      "Fanless Design ไร้พัดลม เงียบสนิท",
      "โครงอลูมิเนียมคุณภาพสูง",
      "รองรับ Windows 10 และ Linux",
    ],
    applications: ["Industrial Automation", "Edge Computing"],
  },
];
