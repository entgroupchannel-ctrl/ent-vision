...
const GlobalFloatingToolbar = () => {
  ...
  return (
    <>
      {/* Floating toolbar - top right, below navbar */}
      <div className="fixed top-20 right-4 z-40 flex flex-col gap-2">
        {/* AI Chat Button */}
        <button
          onClick={() => setAiChatOpen((prev) => !prev)}
          className="group relative w-10 h-10 rounded-full bg-primary text-primary-foreground shadow-lg hover:shadow-xl hover:scale-105 transition-all flex items-center justify-center"
          aria-label="คุยกับ AI"
          title="คุยกับ AI ผู้ช่วย"
        >
          <Bot size={18} />
